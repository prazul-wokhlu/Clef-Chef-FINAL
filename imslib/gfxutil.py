...
from kivy.graphics import Rectangle, Ellipse, Color, Line, BindTexture
...
class Cursor3D(InstructionGroup):
    ...
    def unset(self):
        ...
        # draw offscreen so the cursor disappears.
        self.cursor.csize = (10,10)
        self.cursor.cpos = (-500, -500)
...
def count_canvas_items(canvas):
    """
    Counts all graphics instruction objects in the given canvas by recursively counting children.

    :param canvas: A kivy canvas object (such as `Widget.canvas`)

    :returns: number of items found
    """
    num_items = 0
    for i in canvas.children:
        if isinstance(i, InstructionGroup):
            num_items += count_canvas_items(i)
        elif not isinstance(i, BindTexture):
            num_items += 1
    return num_items
